...
def split_text_around_halfway(A):
...
def recursive_merge_summarises_new(summary_1: str, summary_2: str):
    result="\n# \n"+summary_1+"\n# \n"+summary_2
    return result

def recursive_summarize_text(text: str, filter: str = None, summary_length: int = 500,model="gpt-3.5-turbo"):
    if filter:
    ...
    if n_tokens>4096:
        text_1,text_2=split_text_around_halfway(text)
        summary_1=recursive_summarize_text(text_1, filter, summary_length=summary_length,model=model)
        summary_2=recursive_summarize_text(text_2, filter, summary_length=summary_length,model=model)
        summary=recursive_merge_summarises_new(summary_1, summary_2)
        return summary
    ...
    try:
        ...
        return recursive_summarize_text(text, filter, summary_length,model=model)
